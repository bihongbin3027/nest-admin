...
`auth` / `user` / `menu` / `role` / `perm` / `dept` / `post` / `oss`，每个模块为标准 NestJS 形态 `*.module.ts` + `*.controller.ts` + `*.service.ts` + `*.entity.ts` + `dto/`。
...
- 装饰器：`@AllowAnon()`（无需 Token）、`@AllowNoPerm()`（有 Token 但跳过接口权限校验）位于 `common/decorators/`
...
- 业务视图在 `views/`，按业务域分目录（`dashboard` / `login` / `permission` / `redirect` / `system`）
